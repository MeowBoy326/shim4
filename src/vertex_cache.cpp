...
void Vertex_Cache::cache(SDL_Colour vertex_colours[3], util::Point<float> da, util::Point<float> db, util::Point<float> dc)
{
	...
	// Set vertex x, y
	(*_vertices)[12*(*_count+0)+0] = da.x;
	(*_vertices)[12*(*_count+0)+1] = da.y;
	(*_vertices)[12*(*_count+1)+0] = db.x;
	(*_vertices)[12*(*_count+1)+1] = db.y;
	(*_vertices)[12*(*_count+2)+0] = dc.x;
	(*_vertices)[12*(*_count+2)+1] = dc.y;
...
void Vertex_Cache::cache(SDL_Colour vertex_colours[4], util::Point<float> source_position, util::Size<float> source_size, util::Point<float> da, util::Point<float> db, util::Point<float> dc, util::Point<float> dd, int flags)
{
	...
	(*_vertices)[12*(*_count+2)+0] = dc.x;
	(*_vertices)[12*(*_count+2)+1] = dc.y;
	(*_vertices)[12*(*_count+3)+0] = da.x;
	(*_vertices)[12*(*_count+3)+1] = da.y;
	(*_vertices)[12*(*_count+4)+0] = dc.x;
	(*_vertices)[12*(*_count+4)+1] = dc.y;
	(*_vertices)[12*(*_count+5)+0] = dd.x;
	(*_vertices)[12*(*_count+5)+1] = dd.y;
